...
class ModelAdmin extends Model
{
    ...
    {
        return $this->db->table('tbl_profil_masjid')
            ->where('id', 'pm-1')
            ->get()->getRowArray();
    }
    public function UpdateSetting($data)
    {
        $this->db->table('tbl_profil_masjid')
            ->where('id', 1)
            ->update($data);
    }
    ...
    public function AllDonasi()
    {
        return $this->db->table('tbl_donasi')
            ...
            ->select('tbl_donasi.no_rek as no_rek_tujuan')
            ->select('tbl_donasi.nama_bank as nama_bank_tujuan')
